...
/* ── PRICING ────────────────────────────────────────────────────────────────
   Each model carries its own published starting figure in `priceFrom`. A model
   without one reads "Pricing on request" rather than showing a derived guess.
   Never state or imply a per-square-foot rate anywhere on the site. */
export const startingPrice = (m) => (m && typeof m.priceFrom === 'number' ? m.priceFrom : null);
...
export const PRICE_BASIS = "Includes RiseCon's standard finish and amenity package.";

export const PRICE_DISCLAIMER = "Pricing shown is a starting figure and includes RiseCon's standard selected finishes, fixtures, and amenities. Finishes, fixtures, features, and amenities can be upgraded or customized. Any upgrades, additions, site-specific requirements, or custom selections may increase the final project price.";
